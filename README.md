...
3. Once VS Code is running with the folder hosted out of the container, set the `svcUri` value in the `./environments/environments.ts` config file. This is the host for the API.
4. Run this application by running `yarn start` from the container terminal and navigate to `http://localhost:4200/`. Debug/inspect the application using Chrome Developer Tools (`Ctrl+Shift+I`). The app will automatically reload if you change any of the source files. 
