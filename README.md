...
3. Once VS Code is running with the folder hosted out of the container, set the `svcUri` value in the `./src/environments/environments.ts` config file. This is the host for the API (most likely should be `http://localhost:8081`).
4. Run this application by opening a new VS Code terminal and running `yarn start` and navigate to `http://localhost:4200/`. Debug/inspect the application using Chrome Developer Tools (`Ctrl+Shift+I`). The app will automatically reload if you change any of the source files. 
